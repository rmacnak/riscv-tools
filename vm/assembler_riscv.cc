...
void Assembler::mopr(intptr_t n, Register rd, Register rs1) {
  ASSERT(Supports(RV_Zimop));
  Emit32(EncodeMoprn(n) |
         EncodeFunct12(MOP_R) |
         EncodeRs1(rs1) |
         EncodeFunct3(MOP) |
         EncodeRd(rd) |
...
void Assembler::moprr(intptr_t n, Register rd, Register rs1, Register rs2) {
  ASSERT(Supports(RV_Zimop));
  Emit32(EncodeMoprrn(n) |
         EncodeFunct7(MOP_RR) |
         EncodeRs2(rs2) |
         EncodeRs1(rs1) |
         EncodeFunct3(MOP) |
...
void Assembler::cmop(intptr_t n) {
...
void Assembler::amocasd(Register rd, Register rs2, Address addr,
                        std::memory_order order) {
  ASSERT(addr.offset() == 0);
#if XLEN == 32
  ASSERT((rd.encoding() % 2) == 0);
  ASSERT((rs2.encoding() % 2) == 0);
#endif
  ASSERT(Supports(RV_Zacas));
...
#if XLEN >= 64
void Assembler::amocasq(Register rd, Register rs2, Address addr,
                        std::memory_order order) {
  ASSERT(addr.offset() == 0);
#if XLEN == 64
  ASSERT((rd.encoding() % 2) == 0);
